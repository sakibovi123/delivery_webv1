...
class UpdateEmployeeView(View):
    ...
    def post(self, request, shop_id, emp_id):
        ...
        empId = get_object_or_404(Employee, pk=emp_id)
        if shopId.user == request.user and request.method == "POST":
            empId.full_name = request.POST.get("full_name")
            empId.emp_username = request.POST.get("emp_username")
            empId.emp_pin = request.POST.get("emp_pin")
            empId.confirm_pin = request.POST.get("confirm_pin")
            empId.role = request.POST.get("role")
            empId.is_active = request.POST.get("is_active")
            empId.save()
            if empId.is_active == True:
                empId.is_active = True
                empId.save()
            else:
                empId.is_active = False
                empId.save() 
...
class GETCategoryView(View):
    ...
    def post(self, request, shop_id, cat_id):
        ...
        if shopId.user == request.user and catId is not None:
            catId.delete()
            return redirect(f"")
        else:
...
class UpdateCategoryView(View):
    ...
    def post(self, request, shop_id, cat_id):
        ...
        catId = get_object_or_404(Category, pk=cat_id)
        if shopId.user == request.user and request.method == "POST":
            catId.category_name = request.POST.get("category_name")
            catId.save()
            return redirect()
        else:
            return redirect("")
...
class GETBrandView(View):
    ...
    def post(self, request, shop_id, brand_id):
        shopId = get_object_or_404(Shop, pk=shop_id)
        brandId = get_object_or_404(Brand, pk=brand_id)
...
class UpdateBrandView(View):
    ...
    def post(self, request, shop_id, brand_id):
        shopId = get_object_or_404(Shop, pk=shop_id)
        brandId = get_object_or_404(Brand, pk=brand_id)
        if shopId.user == request.user and brandId != "" and request.method == "POST":
            brandId.brand_name = request.POST.get("brand_name")
            brandId.shop = shopId.id
            brandId.save()
            return redirect(f"")
        else:
            return redirect(f"")
...
class GETItemView(View):
    ...
    """ Deleting method item"""
    def post(self, request, shop_id, item_id):
        shopId = get_object_or_404(Shop, pk=shop_id)
        itemId = get_object_or_404(Item, pk=item_id)
        if itemId.id != "":
            itemId.delete()
            return redirect(f"")
        else:
